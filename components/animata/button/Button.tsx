...
export default function RippleButton({ children, className, textClassName, ...props }: RippleButtonProps) {
  ...
  return (
    <button
      ref={buttonRef}
      className={cn(
        "font-jost duration-[600ms] relative flex items-center justify-center overflow-hidden rounded-full bg-[#cbfe7e] p-[1.3rem] text-[1.2rem] font-medium text-[#0e352e] transition",
        className,
      )}
      onMouseEnter={(e) => {
        createRipple(e);
      ...
      {...props}
    >
      <span className={cn("relative z-[2]", textClassName)}>{children}</span>
      ...
      <style>{`
        .ripple {
          ...
          border-radius: 50%;
          pointer-events: none;
          background-color: rgba(212, 163, 27, 0.6); /* deeper amber gold */
          box-shadow: 0 0 24px rgba(212, 163, 27, 0.4), 0 0 48px rgba(212, 163, 27, 0.25);
          z-index: 1;
          opacity: 0;
